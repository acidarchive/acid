...
async fn get_patterns_tb303_random_returns_a_random_pattern() {
    ...
    let post_response = app.post_patterns_tb303(body.into(), token).await;
    ...
    assert_eq!(json["data"]["title"], "Stakker humanoid");
    assert_eq!(json["data"]["author"], "Humanoind");
    assert_eq!(json["data"]["waveform"], "sawtooth");
    assert_eq!(json["data"]["bpm"], 130);
    assert!(json["data"]["steps"].is_array());
    assert_eq!(json["data"]["steps"].as_array().unwrap().len(), 16);
}
